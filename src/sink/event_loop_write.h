...
template <class EV>
struct EventLoopWriteState
{
    EV &ev;
};

template <class EV, class T>
struct EventLoopWriteNode
{
    ...
    EventLoopWriteState<EV> state;
};

template <class T>
struct EventLoopWrite : BaseSink<EventLoopWriteNode<T>, EventLoopWriteState<T>>
{
};
